...
then
  mkdir -p images
  rm -rf images/*
  docker save ${repo}-orthanc:${tag} | gzip > images/orthanc.gz.tar
  #docker save ${repo}-worker:${tag} | gzip > images/worker.gz.tar
  docker save ${repo}-scheduler:${tag} | gzip > images/scheduler.gz.tar
fi